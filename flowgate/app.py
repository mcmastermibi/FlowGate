...
class FlowGateApp:
    ...
    def _lbl(self, parent, text, size=9, bold=False, color=FG_TEXT) -> tk.Label:
        font = ("Helvetica", size, "bold" if bold else "normal")
        return tk.Label(parent, text=text, bg=BG_PANEL, fg=color, font=font)

    def _btn(self, parent, text, cmd, color=ACC_BLUE, width=16) -> tk.Button:
        return tk.Button(
            parent, text=text, command=cmd,
            bg=color, fg=BG_DARK, activebackground=FG_TEXT,
    ...
    def _build_left_panel(self, parent):
        # File info
        sec = tk.Frame(parent, bg=BG_PANEL)
        sec.pack(fill=tk.X, padx=10, pady=(12, 4))
        self._lbl(sec, "FILE", size=8, color="#666980").pack(anchor=tk.W)
        self.file_label = self._lbl(sec, "No file loaded", size=9, color=ACC_BLUE)
        self.file_label.pack(anchor=tk.W)
        self._btn(sec, "Open FCS…", self.open_fcs, width=14).pack(anchor=tk.W, pady=(6, 0))

        tk.Frame(parent, bg=SEP_COLOR, height=1).pack(fill=tk.X, padx=10, pady=8)

        # Channel selectors
        sec2 = tk.Frame(parent, bg=BG_PANEL)
        sec2.pack(fill=tk.X, padx=10, pady=4)
        self._lbl(sec2, "AXES", size=8, color="#666980").pack(anchor=tk.W)

        # X axis row
        row_x = tk.Frame(sec2, bg=BG_PANEL)
        row_x.pack(fill=tk.X, pady=2)
        self._lbl(row_x, "X:", size=9).pack(side=tk.LEFT)
        ...
        # X transform + cofactor
        row_xt = tk.Frame(sec2, bg=BG_PANEL)
        row_xt.pack(fill=tk.X, pady=(0, 4))
        self._lbl(row_xt, "  ", size=9).pack(side=tk.LEFT)  # indent
        self.x_transform_var = tk.StringVar(value="asinh")
        for t in ("asinh", "linear"):
            rb = tk.Radiobutton(row_xt, text=t, variable=self.x_transform_var,
                                value=t, bg=BG_PANEL, fg=FG_TEXT,
                                selectcolor=BG_DARK, activebackground=BG_PANEL,
                                font=("Helvetica", 8), command=self.on_transform_change)
            rb.pack(side=tk.LEFT, padx=1)
        self._lbl(row_xt, "cf:", size=8, color="#888A9A").pack(side=tk.LEFT, padx=(6, 2))
        self.x_cofactor_var = tk.StringVar(value="150")
        ex = tk.Entry(row_xt, textvariable=self.x_cofactor_var, width=5,
        ...
        # Y axis row
        row_y = tk.Frame(sec2, bg=BG_PANEL)
        row_y.pack(fill=tk.X, pady=2)
        self._lbl(row_y, "Y:", size=9).pack(side=tk.LEFT)
        ...
        # Y transform + cofactor
        row_yt = tk.Frame(sec2, bg=BG_PANEL)
        row_yt.pack(fill=tk.X, pady=(0, 4))
        self._lbl(row_yt, "  ", size=9).pack(side=tk.LEFT)  # indent
        self.y_transform_var = tk.StringVar(value="asinh")
        for t in ("asinh", "linear"):
            rb = tk.Radiobutton(row_yt, text=t, variable=self.y_transform_var,
                                value=t, bg=BG_PANEL, fg=FG_TEXT,
                                selectcolor=BG_DARK, activebackground=BG_PANEL,
                                font=("Helvetica", 8), command=self.on_transform_change)
            rb.pack(side=tk.LEFT, padx=1)
        self._lbl(row_yt, "cf:", size=8, color="#888A9A").pack(side=tk.LEFT, padx=(6, 2))
        self.y_cofactor_var = tk.StringVar(value="150")
        ey = tk.Entry(row_yt, textvariable=self.y_cofactor_var, width=5,
                      bg=BG_DARK, fg=FG_TEXT, insertbackground=FG_TEXT,
                      font=("Helvetica", 8), relief=tk.FLAT)
        ...
        # Gating tools
        sec3 = tk.Frame(parent, bg=BG_PANEL)
        sec3.pack(fill=tk.X, padx=10, pady=4)
        self._lbl(sec3, "GATING TOOLS", size=8, color="#666980").pack(anchor=tk.W, pady=(0, 6))

        self.parent_label = self._lbl(sec3, "Parent: All Events", size=8, color=ACC_GREEN)
        self.parent_label.pack(anchor=tk.W, pady=(0, 4))

        btn_row = tk.Frame(sec3, bg=BG_PANEL)
        ...
        self._btn(sec3, "✕  Cancel Draw", self.cancel_draw, color="#444455", width=16).pack(
            anchor=tk.W, pady=(4, 0))

        tk.Frame(parent, bg=SEP_COLOR, height=1).pack(fill=tk.X, padx=10, pady=8)

        # Gate hierarchy tree
        sec4 = tk.Frame(parent, bg=BG_PANEL)
        sec4.pack(fill=tk.BOTH, expand=True, padx=10, pady=4)
        self._lbl(sec4, "GATE HIERARCHY", size=8, color="#666980").pack(anchor=tk.W)

        tree_frame = tk.Frame(sec4, bg=BG_DARK)
        ...
        style.configure("Flow.Treeview",
                        ...
                        fieldbackground=BG_DARK, font=("Helvetica", 9),
                        rowheight=22)
        style.configure("Flow.Treeview.Heading",
                        background=BG_PANEL, foreground="#666980",
                        font=("Helvetica", 8, "bold"))
        style.map("Flow.Treeview",
                  background=[("selected", "#1E3A5F")],
                  foreground=[("selected", ACC_BLUE)])
        ...
        # Gate action buttons
        gbtn = tk.Frame(sec4, bg=BG_PANEL)
        gbtn.pack(fill=tk.X, pady=(6, 0))
        self._btn(gbtn, "⊕ Gate In", self.gate_into_selected,
                  color=ACC_GREEN, width=9).pack(side=tk.LEFT, padx=(0, 4))
        self._btn(gbtn, "✎ Rename", self.rename_gate,
                  color="#555566", width=9).pack(side=tk.LEFT, padx=(0, 4))
        self._btn(gbtn, "✕ Delete", self.delete_selected_gate,
                  color=ACC_RED, width=9).pack(side=tk.LEFT)
    ...
    def _build_bottom_bar(self, parent):
        ...
        tk.Label(bar, textvariable=self.status_var, bg=BG_PANEL, fg="#888A9A",
                 font=("Helvetica", 8), anchor=tk.W).pack(side=tk.LEFT, padx=10)

        self.event_count_var = tk.StringVar(value="")
        tk.Label(bar, textvariable=self.event_count_var, bg=BG_PANEL, fg=ACC_BLUE,
    ...
    def _draw_placeholder(self):
        ...
        self.ax.text(0.5, 0.5, "Open an FCS file to begin",
                     ...
                     color="#444455", fontsize=14, fontstyle="italic")
        self.ax.set_xticks([])
        self.ax.set_yticks([])
    ...
    def refresh_plot(self):
        ...
        self.ax.scatter(
            ...
            c="#4DAACC", rasterized=True, linewidths=0,
        )
        ...
        self.ax.set_title(title, color=ACC_BLUE, fontsize=10, pad=6)
    ...
    def _draw_gate_overlay(self, gate: Gate):
        if gate.gate_type == "polygon" and len(gate.vertices) >= 3:
            ...
            self.ax.text(cx, cy, gate.name, color=gate.color,
                         ...
                         bbox=dict(boxstyle="round,pad=0.2", fc=BG_DARK, ec=gate.color,
                                   lw=0.6, alpha=0.8))
